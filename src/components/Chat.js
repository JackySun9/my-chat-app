...
const Chat = () => {
  ...
  const MESSAGE_LIMIT = 400; // Character limit for displaying "Read More"
  ...
  const sendMessage = async () => {
    ...
    const inputMessage = input; // Store input message
    setInput(""); // Clear input field immediately

    // Fetch response from the AI model using the selected model
    await fetchResponseFromAI(inputMessage, selectedModel);
  };
